...
# @Last Modified time: 2022-01-13 13:02:07
...
numbers = pd.DataFrame(columns = ['n', 'envs'])


for s in info.index[1:5]:
# for s in [info.index[0:4][-1]]:
	...
	for i in range(len(idx_eps)):
		...
		numbers.loc[s+'_'+str(i),'envs'] = env[idx_eps[i]].values
		...
		if len(neurons)>50:
			...
			pos[s+'_'+str(i)] = [position[['x', 'z']].restrict(wake_ep.loc[[j]]) for j in idx_eps[i]]
			# fig = figure()
			...
			ratio_dist[s+'_'+str(i)] = pd.Series(index = bins[0:-1], data = xs/(xs+xc))
...
sess = np.array(list(p3d.keys()))[[0,2,3]]
...
for i,s in enumerate(sess):
	gs2 = GridSpecFromSubplotSpec(1,3,gs[0,i])
	for j, e in enumerate(numbers.loc[s,'envs']):
		subplot(gs2[0,j])
		plot(pos[s][j]['x'], pos[s][j]['z'], color = cmap(np.linspace(0,1,3)[j]))
